...
export default function App() {
  ...
  const handleSave = (titulo, paginas, dias) => {
    if (!titulo || !paginas || !dias) return Alert.alert('Erro', 'Preencha todos os campos');
    const paginasNum = parseFloat(paginas), diasNum = parseFloat(dias);
    if (isNaN(paginasNum) || isNaN(diasNum) || paginasNum < 0 || diasNum < 0) return Alert.alert('Erro', 'Valores inválidos');

    if (registroEmEdicao) {
      setRegistros(registros.map(r => r.id === registroEmEdicao.id ? { ...r, titulo, paginas: paginasNum, dias: diasNum } : r));
      Alert.alert('Sucesso', 'Registro atualizado!');
    } else {
      setRegistros([...registros, { id: new Date().getTime(), titulo, paginas: paginasNum, dias: diasNum }]);
  ...
  if (ordenacao === 'maior_paginas') registrosExibidos.sort((a,b)=>b.paginas-a.paginas);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <Text style={styles.titulo}>Diário de Leitura 📚</Text>
        ...
        <View style={{flexDirection:'row', justifyContent:'center', marginBottom:10, gap:10}}>
          ...
          <Button title="Maior Páginas" onPress={()=>setOrdenacao('maior_paginas')}/>
        </View>
